...
Keycloak needs a database to store its data. We'll use Postgres. Back on the {openshift_cluster_console}/topology/ns/{{ USER_ID }}-project?view=graph[OpenShift web console^], click the Add icon once again, type `postgres ephemeral` into the search box, locate the `Postgres (Ephemeral)` template, click on it, and click *Instantiate Template*:

image::postgres-create-ks.png[]
...
image::postgres-create2-ks.png[]

You should see a postgres database begin spinning up:

image::postgres-create3-ks.png[]

=== Create self-signed certificate

In the DevSpaces window, Open a Terminal window.

image::new-terminal.png[new-terminal, 500]

Let's make the postgresql database icon show its proper logo:
...
Back on the {openshift_cluster_console}/topology/ns/{{ USER_ID }}-project?view=graph[OpenShift web console^] topology page, click the small "open" icon at the upper right of the Keycloak deployment:

image::keycloak-openicon.png[]

You should land on the Keycloak intro screen:

image::admin-intro.png[]

Click *Administration Console* to log in with the following:

* *Username or email*: `admin`
* *Password*: The password you retrieved from the {openshift_cluster_console}/k8s/ns/{{ USER_ID }}-project/secrets/keycloak-initial-admin[secret^] earlier.
...
----
mp.jwt.verify.publickey.location=keycloak-{{ USER_ID }}-project.{{ROUTE_SUBDOMAIN}}/realms/quarkus/protocol/openid-connect/certs<1>
mp.jwt.verify.issuer=keycloak-{{ USER_ID }}-project.{{ROUTE_SUBDOMAIN}}/realms/quarkus<2>
...
----
export ALICE_TOKEN=$(\
    curl -ks -X POST keycloak-{{ USER_ID }}-project.{{ROUTE_SUBDOMAIN}}/realms/quarkus/protocol/openid-connect/token \
...
Injected issuer: ["keycloak-{{ USER_ID }}-project.{{ROUTE_SUBDOMAIN}}/realms/quarkus"]
...
----
# OIDC config
quarkus.oidc.auth-server-url=keycloak-{{ USER_ID }}-project.{{ROUTE_SUBDOMAIN}}/realms/quarkus
